...
util.widget_background = function(widget, bg, label_widget, label_bg)
	if label_widget == nil then
		return {
			wibox.container.background(wibox.container.margin(widget, 6, 6, 5, 5), bg, util.shape),
			layout = wibox.layout.fixed.vertical
		}
	else
		return {
			wibox.container.background(wibox.container.margin(util.center(label_widget), 0, 0, 2, 2), label_bg, util.top_half_shape),
			wibox.container.background(wibox.container.margin(                   widget, 6, 6, 5, 5),       bg, util.bot_half_shape),
			layout = wibox.layout.fixed.vertical
		}
	end
end

util.spacer = wibox.container.margin(nil, 0, 0, 10, 0)
