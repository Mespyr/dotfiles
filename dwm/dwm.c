	...
	if (m->lt[m->sellt]->arrange)
		...
		for (c = selmon->clients; c; c = c->next)
			if (ISVISIBLE(c) && c->bw == 0)
				resize(c, c->x, c->y, c->w - 2*borderpx, c->h - 2*borderpx, 0, 0);
}
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		for (c = m->clients; c; c = c->next)
			occ |= c->tags;
		// inc x for sizeof potential button here
		do
		...
		if (i < LENGTH(tags)) {
			click = ClkTagBar;
			arg.ui = 1 << i;
		} else if (ev->x < x + TEXTW(selmon->ltsymbol)) // remember `x + TEXTW(selmon->ltsymbol)`
			click = ClkLtSymbol;
...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	w += horizpadbar / 2; /* padding on both sides */
	ret = x = m->ww - w;
	...
	while (text[++i]) {
		if (text[i] == '^' && !isCode) {
			...
			text[i] = '\0';
			w = TEXTW(text) - lrpad;
			drw_text(drw, x, statusvertpadbar / 2, w, bh - statusvertpadbar, 0, text, 0);
			x += w;
			...
			while (text[++i] != '^') {
				if (text[i] == 'c') {
					...
					drw_rect(drw, rx + x, ry + statusvertpadbar / 2, rw, rh, 1, 0);
				} else if (text[i] == 'f') {
					x += atoi(text + ++i);
				}
			}
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		// draw indicator for item in focus
		if (m->tagset[m->seltags] & 1 << i)
			drw_rect(drw, x + seltagindpad, bh-seltagindheight, w - (2*seltagindpad), seltagindheight, 1, 0);
		x += w;
	}
...
centeredmaster(Monitor *m)
{
	...
	// border pixels for one win vs multiple
	if (n == 1) bw = 0;
	else bw = borderpx;

	/* initialize areas */
	mw = m->nmaster ? m->ww * m->mfact : 0; // master width
	mx = (m->ww - mw) / 2; // master pos x
	my = 0; // master pos y
	tw = (m->ww - mw) / 2; // slave client widths

	oty = 0;
	ety = 0;
	for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
	if (i < m->nmaster) { // if a master client
